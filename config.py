...
LATEST_FULL_SEC = 120           # 최신 항목이라도 이보다 오래되면 전문 대신 압축본만 보낸다
...
# 창 제목이 다른 곳(예: "루미 AI 설정")에 가 있어도 화면에 작업 내용이 떠 있으면 전문을 보낸다.
...
EPISODE_INJECT_COUNT = 3         # 시스템 메시지에 붙일 최근 회고(episodes.jsonl) 수
...
- 해요체로 쓴다. 두세 문장.
- 그날 있었던 일 중 하나를 콕 집어 말한다. 하루를 고르게 나열하지 않는다.
- 기록에 없는 일은 지어내지 않는다.
...
{{"summary": "두세 문장", "keywords": ["단어", "단어", "단어"]}}"""
